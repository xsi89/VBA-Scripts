...
'Mini: 400,00
    ...
    For Each cell In Range("N1:N" & ActiveWS)
        ...
        IDresultNFR = IDLNoMatchRes + IDLFuzzyRes + IDRepRes
      ...
      '  MsgBox "NoMatch Words: " & IDNoMatchRes & vbNewLine & "Fuzzy Words: " & IDFuzzyRes & vbNewLine & "100 & Reps Words: " & IDRepRes
        ...
        IDFinalResult = IDResultPQR - IDresultNFR
        
        
       ' MsgBox IDFinalResult
...
Sub calc()
        ...
        For Each c In rng
            If c.Value <> "" Then
                r = c.row
                c.Value = (Range("L" & r) * 1.35 + Range("M" & r) * 0.67 + Range("N" & r) * 0.34) _
                        - (Range("P" & r) + Range("Q" & r) + Range("R" & r))
            End If
        Next
End Sub
