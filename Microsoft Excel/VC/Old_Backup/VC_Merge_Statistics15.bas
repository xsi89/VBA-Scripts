...
End Sub
Sub myyear()
'This function removes the 6 last characters from the word and then replace with the first 4 characters.
...
'                     MsgBox ws2.Range("F" & s).Value
                  ...
                  ws1.Range("O" & r).Value = Val(ws1.Range("K" & r)) * Val(ws2.Range("F" & s))
...
For Each cell In Range("J1:J" & ActiveWS) 'define range of cells
    If InStr(1, cell.Value, "", vbTextCompare) > 0 Then ' if zero
    myresult = Val(cell.Value * 1.35) ' math formula
    Range("N" & cell.Row).Value = myresult 'result of math formula + cellvalue also do add in column P
    End If
Next
...
For Each cell In Range("K1:K" & ActiveWS) 'define range of cells
    If InStr(1, cell.Value, "", vbTextCompare) > 0 Then ' if zero
     myresult = Val(cell.Value * 0.67) ' math formula
    Range("O" & cell.Row).Value = myresult 'result of math formula + cellvalue also do add in column P
    End If
Next
...
'                  Dim myval3 As Integer
                  ...
                  ws1.Range("O" & r).Value = Val(ws1.Range("K" & r)) * Val(ws2.Range("G" & s))
...
For i = 2 To ActiveSheet.UsedRange.SpecialCells(xlCellTypeLastCell).Row
    If Cells(i, "H") = mystring1 And Cells(i, "I") = mystring2 And Len(Cells(i, "B")) <> 0 And Cells(i, "AB") <> 0 Then
    Cells(i, 1).Select
...
Sub SameTest()
Dim LastHRow As Double
Dim RowCtr As Double

LastHRow = Cells(Rows.Count, "H").End(xlUp).Row

    For RowCtr = 1 To LastHRow
        If Cells(RowCtr, "H") = Worksheets("volvo_NewPrices").Cells(RowCtr, "A") And _
            Cells(RowCtr, "I") = Worksheets("volvo_NewPrices").Cells(RowCtr, "B") Then
            MsgBox "Same"
            Exit Sub
        End If
    Next RowCtr
